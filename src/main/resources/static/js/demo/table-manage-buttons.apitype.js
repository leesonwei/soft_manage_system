	...
	action: function ( e, dt, node, config ) {
		layer.open({
			...
			yes:function(index1, layero){
                ...
                target.apiTypeName = layero.find('#apiTypeName').val();
                if (target.apiTypeName === undefined || '' === target.apiTypeName) {
                    layer.msg("API菜單名稱不能為空", {
                        icon:2,
                        time: 3000,
                    });
                    layero.find('#apiTypeName').focus();
                    return;
                }
                target.apiTypeMemo = $(layero.find('#apiTypeMemo')).val();
                ...
                layer.confirm('您確定保存這條記錄嗎？',{
                    ...
                    PublicFunc.ajaxCRUD(target,'/admin/apitype/insert',function(ret){
...
$.fn.dataTable.ext.buttons.edit = {
	...
	action: function ( e, dt, node, config ) {
        ...
        layer.open({
            ...
            btn: ['保存', '取消'],
            success:function(layero, index){
                layero.find('#id').val(target.apiTypeId);
                layero.find('#apiTypeName').val(target.apiTypeName);
                layero.find('#apiTypeMemo').val(target.apiTypeMemo);
            },
            yes:function(index1, layero){
                layer.confirm('您確定保存所有的修改嗎？',{
                    ...
                    layero.find('a.layui-layer-btn0').text('正在提交...');
                    PublicFunc.ajaxCRUD(target,'/admin/apitype/update',function(ret){
...
$.fn.dataTable.ext.buttons.delete = {
	...
	action: function ( e, dt, node, config ) {
        ...
        layer.confirm('您確定刪除這條記錄嗎？',{
            ...
            PublicFunc.ajaxCRUD(target,'/admin/apitype/delete',function(ret){
